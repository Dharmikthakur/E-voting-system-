...
export default function AdminDashboard() {
  ...
  const fetchAdminData = async () => {
    try {
      ...
      const [statsRes, candRes] = await Promise.all([
        ...
        axios.get('http://localhost:5000/api/voting/candidates')
      ]);
  ...
  const handleToggleElection = async () => {
    try {
      ...
      const res = await axios.post(`http://localhost:5000/api/admin/election/${action}`, {}, {
  ...
  const handleAddCandidate = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post('http://localhost:5000/api/admin/candidate', newCandidate, {
        headers: { Authorization: `Bearer ${token}` }
      });
